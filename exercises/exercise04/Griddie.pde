...
class Griddie {
  ...
  void update() {
    ...
    //ANSWER; the move type randomizes a unit of movement between -1 and 2 on its XY axis
    //and assigns it to the XY coordinates of the griddies by multiplying it with the size.
    ...
    // ANSWER; The if statements makes the griddies bounce off the edges of the window
    // so that they dont go off the screen
    if (x < 0) {
      x += width;
    }
    else if (x >= width) {
  ...
  void collide(Griddie other) {
    // QUESTION: What is this if-statement for?
    // ANSWER ;  If the energy of the griddies are equal to 0, the statement will return to the
    // beginning of the function and not execute the rest
    if (energy == 0 || other.energy == 0) {
      return;
    }
    
    // QUESTION: What does this if-statement check?
    // ANSWER; If the XY coordinates of a given griddle overlaps with the XY cooordinates
    // of another griddle, the energy of the griddle will increase by the collideEnergy.
  ...
  void display() {
    ...
    //ANSWER; The fill spceifies the fill color of the griddles and their opacity value through the energy variable
    fill(fill, energy); 
    noStroke();
    rect(x, y, size, size);
  }
